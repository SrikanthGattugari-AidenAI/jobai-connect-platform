...
const mockJobs = [
  { id: "job-1", title: "Frontend Developer", company: "TechCorp" },
  { id: "job-2", title: "Backend Engineer", company: "DataSystems Inc." },
  { id: "job-3", title: "UX/UI Designer", company: "CreativeMinds" },
  { id: "job-4", title: "DevOps Engineer", company: "CloudSolutions" },
  { id: "job-5", title: "Data Scientist", company: "AnalyticsPro" }
];
...
const CandidateMatching = () => {
  ...
  // Get selected job from jobId
  const selectedJob = jobId 
    ? mockJobs.find(job => job.id === jobId) || mockJobs[0]
    : mockJobs[0];
  
  // If an applicantId is provided, select that candidate
  useState(() => {
    if (applicantId) {
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
          <div>
            ...
            <p className="text-muted-foreground">
              View and manage candidates for <span className="font-medium">{selectedJob.title}</span>
            </p>
          </div>
          
          <div className="mt-4 md:mt-0 w-full md:w-auto">
            <select 
              className="w-full md:w-60 rounded-md border border-input bg-background px-3 py-2 text-sm"
              value={selectedJob.id}
              onChange={(e) => navigate(`/employer/candidates?jobId=${e.target.value}`)}
            >
              {mockJobs.map(job => (
